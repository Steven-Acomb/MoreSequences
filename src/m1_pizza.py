...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    pointList = generate_points_on_circle(circle,number_of_points)
    for k in range(number_of_points):
        circleA = rg.Circle(pointList[k],10)
...
def run_test_pizza():
    ...
    title = 'PIZZA test 1:  5 slices, thin (thickness=3) blue lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(195, 205), 150)
    circle.outline_thickness = 3
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    pointList = generate_points_on_circle(circle, number_of_slices)
    for k in range(number_of_slices):
        lineA = rg.Line(pointList[k], circle.center)
        lineA.thickness = thickness
        lineA.color = color
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(350, 200), 150)
    circle.outline_thickness = 3
    polygon(window, circle, 6, 'green', 8)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments-1):
        lineA = rg.Line(pointList[k],pointList[k+1])
        lineA.thickness = thickness
        lineA.color = color
        lineA.attach_to(window)
        window.render()
    lineA = rg.Line(pointList[number_of_segments-1], pointList[0])
    lineA.thickness = thickness
    lineA.color = color
    lineA.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  8 black lines on white circle, ' \
            'hops = 2.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'white'
    fancy_polygon(window, circle, 8, 2, 'black', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    ####################################################################
    # ------------------------------------------------------------------
    pointList = generate_points_on_circle(circle, number_of_lines)
    circle.attach_to(window)
    for k in range(number_of_lines):
        lineA = rg.Line(pointList[k], pointList[(k + hops_to_next_point) %
                                                  number_of_lines])
        lineA.thickness = thickness
        lineA.color = color
        lineA.attach_to(window)
